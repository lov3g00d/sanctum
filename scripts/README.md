...
Hardened operational Bash for the Sanctum platform. Every script uses strict mode
...
of the podinfo contract on port 9898 (see `shared/docker/README.md`), so the probe works
against podinfo directly.

```sh
./health-check.sh https://api.sanctum.example.com 5
```
...
| `S3_PREFIX` | no | `sanctum/postgres` | Key prefix |
...
`sanctum/postgres/<db>/YYYY/MM/DD/HHMMSS.sql.gz`.

```sh
# /etc/cron.d/sanctum-pg-backup  (env from an EnvironmentFile or Secrets Manager)
15 2 * * * ops /opt/sanctum/scripts/backup-postgres.sh
```
