...
Deployed by the platform module, `terraform/modules/platform/trivy-operator.tf`,
...
The values that matter for Nimbus: scan every namespace except `kube-system` and
...
`terraform/modules/platform/policies/cspm/kube-bench-job.yaml`, which audits node
