...
`redteam/attacks/`, names the Nimbus control that stops each one, and states how
...
platform module on purpose (see `redteam/README.md`).
...
| Initial Access | T1610 Deploy Container (supply chain) | `05-unsigned-image.yaml` | Kyverno `verify-image-signatures` (Enforce) | Admission rejects the unsigned nimbus-ECR image; a policy-clean pod template isolates the signature check as the sole reason |
| Credential Access | T1552.001 Credentials In Files | `06-shadow-read.yaml` | Tetragon `nimbus-block-credential-access` (SIGKILL) + Falco sensitive-file read | Pod is admitted, then the `/etc/shadow` read is SIGKILLed (exit 137) and the container CrashLoops |
...
| Lateral Movement | T1021 Remote Services | `08-lateral-movement.yaml` | default-deny egress NetworkPolicy (nimbus) | Pod is admitted; the cross-namespace / control-plane connect is dropped |
...
`run-validation.sh` reads the `redteam.nimbus/control-class` label off each
...
- **T1610 supply chain (05).** Kyverno `verify-image-signatures` only matches
  `*.dkr.ecr.eu-central-1.amazonaws.com/nimbus/*`. A generic public image is out
  ...
  attempt therefore references the nimbus ECR repo with an unsigned tag so the
...
attacker tries and the earliest Nimbus layer that ends the attempt.

| Phase | Attacker goal | Nimbus response | Where |
|-------|---------------|-----------------|-------|
...
| cosign verifyImages | 05 | An unsigned nimbus-ECR image is refused at admission |
