...
outline for Sanctum. It ties the abstract question ("is this secure?") to
...
Sanctum is a B2B API platform (see [`00-scenario.md`](00-scenario.md)). Partners
integrate against `api.sanctum.example.com`, so a compromise is not an internal
...
|--------|--------|---------|-------|
| Denial of service | Volumetric / L7 flood | Cloudflare DDoS + rate limiting; HPA/Karpenter absorb legitimate spikes | `chambers/aws-eks/terraform/modules/cloudflare/`, `shared/charts/podinfo/templates/hpa.yaml` |
| Tampering | Origin bypass to hit ALB directly | Authenticated origin pull (mTLS) edge to ALB; WAFv2 on the ALB | scenario B2, ALB ingress `shared/charts/podinfo/templates/ingress.yaml` |
...
| EoP | SSRF pivoting into the VPC | Default-deny egress NetworkPolicy limits where a pod can reach | `chambers/aws-eks/terraform/modules/platform/policies/cluster/networkpolicy.yaml` |

### B4 pod to data

| Threat | Vector | Control | Where |
|--------|--------|---------|-------|
| Lateral movement | Compromised pod scanning the namespace | Default-deny ingress/egress, explicit allows only to RDS/Redis CIDR | `chambers/aws-eks/terraform/modules/platform/policies/cluster/networkpolicy.yaml` |
| EoP | Pod reading other secrets via API | RBAC scoped by `resourceNames` to the workload's own ConfigMap/Secret | `chambers/aws-eks/terraform/modules/platform/policies/cluster/rbac.yaml` |
| Info disclosure | Credential theft from the pod | IRSA short-lived creds, not node role; secrets from Secrets Manager, never in Git | `shared/charts/podinfo/templates/serviceaccount.yaml`, `chambers/aws-eks/terraform/modules/platform/README.md` |
| Spoofing | Stolen SA token used against the API server | Falco alerts on token read by a shell | `chambers/aws-eks/terraform/modules/platform/config/falco-rules.yaml` |
| Repudiation | Post-compromise persistence/tampering | Read-only rootfs; Falco alerts on writes to sensitive dirs and shell spawn | PSA restricted + `chambers/aws-eks/terraform/modules/platform/config/falco-rules.yaml` |

### B5 supply chain

| Threat | Vector | Control | Where |
|--------|--------|---------|-------|
| Tampering | Malicious image pushed to ECR | Kyverno `verify-images` admits only images signed by the CI OIDC identity (Enforce) | `chambers/aws-eks/terraform/modules/platform/policies/kyverno/verify-images.yaml` |
...
| Provenance gap | No verifiable bill of materials at admission | SBOM attestation policy (Audit, aspirational; needs `cosign attest` in CI) | `chambers/aws-eks/terraform/modules/platform/policies/kyverno/require-signed-and-sbom.yaml` |

### B6 control plane

| Threat | Vector | Control | Where |
|--------|--------|---------|-------|
| Spoofing | Leaked long-lived AWS keys | GitHub OIDC, no static keys; trust scoped to repo + ref | `.github/workflows/README.md`, `chambers/aws-eks/terraform/modules/github-oidc/` |
| EoP | CI role too broad | Split roles per stage (deploy / tf-plan / tf-apply), least privilege | `.github/workflows/cd.yml`, `chambers/aws-eks/terraform/` |
| Tampering | Manual console change bypassing IaC | Prowler CSPM catches account drift; kube-bench catches cluster drift | `chambers/aws-eks/terraform/modules/platform/policies/cspm/` |
...
| Signature verification | `chambers/aws-eks/terraform/modules/platform/policies/kyverno/verify-images.yaml` | Unsigned/foreign image scheduled | **Enforce** |
| SBOM attestation | `chambers/aws-eks/terraform/modules/platform/policies/kyverno/require-signed-and-sbom.yaml` | No verifiable BOM at admission | **Audit (aspirational)** |
| Pod Security Admission | `chambers/aws-eks/terraform/modules/platform/policies/cluster/namespace.yaml` | Privileged/root/host-mount pods | Enforced (`restricted`) |
| Pod hardening policies | `chambers/aws-eks/terraform/modules/platform/policies/kyverno/pod-security-policies.yaml` | `:latest`, root, writable rootfs, no limits, extra caps | Enforced |
| Network segmentation | `chambers/aws-eks/terraform/modules/platform/policies/cluster/networkpolicy.yaml` | Lateral movement, unexpected egress | Enforced (default-deny) |
| Least-privilege RBAC | `chambers/aws-eks/terraform/modules/platform/policies/cluster/rbac.yaml` | Token enumerating other secrets | Enforced (namespaced, by name) |
| Workload identity | `shared/charts/podinfo/templates/serviceaccount.yaml` (IRSA) | Node-wide credential blast radius | Enforced |
| Runtime detection | `chambers/aws-eks/terraform/modules/platform/config/falco-rules.yaml` | Shell, exfil, tampering, token theft | Detect (alert) |
| DAST | `.github/workflows/cd.yml` (ZAP baseline) | Runtime exposure static analysis misses | Report-only |
| Cluster CIS audit | `chambers/aws-eks/terraform/modules/platform/policies/cspm/kube-bench-job.yaml` | Node/cluster misconfig vs CIS EKS | Audit (on demand) |
| Continuous cluster posture | `docs/trivy-operator.md` | Running-image CVE, config/RBAC drift | Detect (continuous) |
| AWS CSPM | `chambers/aws-eks/terraform/modules/platform/policies/cspm/prowler-cronjob.yaml` | Account drift vs CIS AWS Foundations | Audit (weekly) |
| Edge protection | `chambers/aws-eks/terraform/modules/cloudflare/` | DDoS, L7 attacks, bots | Enforced |
...
|----------|--------------|
| 4.x worker node config | kube-bench `node` target (`chambers/aws-eks/terraform/modules/platform/policies/cspm/kube-bench-job.yaml`) |
| 5.1 RBAC and service accounts | `chambers/aws-eks/terraform/modules/platform/policies/cluster/rbac.yaml`, scoped SA |
| 5.2 Pod Security Standards | PSA `restricted` (`chambers/aws-eks/terraform/modules/platform/policies/cluster/namespace.yaml`) + Kyverno pod-hardening policies |
| 5.3 Network policies | `chambers/aws-eks/terraform/modules/platform/policies/cluster/networkpolicy.yaml` default-deny |
...
Covered by Prowler (`chambers/aws-eks/terraform/modules/platform/policies/cspm/prowler-cronjob.yaml`, `--compliance
...
| 4.x networking | Security groups and NACLs in `chambers/aws-eks/terraform/modules/vpc/`; Prowler checks exposure |

### SOC2 and PCI relevance

Sanctum is not claiming certification here; this maps the controls to the trust
...
- **Malicious image.** `verify-images` should already have blocked an unsigned image; if a signed-but-bad image shipped, revoke by removing the tag, and roll back: `kubectl -n sanctum rollout undo deployment/podinfo` (see `.github/workflows/README.md`).
