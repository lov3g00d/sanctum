...
Observability for Sanctum: how we know the platform is healthy, how we find out
...
under [`chambers/aws-eks/terraform/modules/platform/config/`](../chambers/aws-eks/terraform/modules/platform/config)
...
dashboard ships with the app in [`shared/charts/podinfo`](../shared/charts/podinfo) (delivered
...
SLI recording rules ship with the app (`shared/charts/podinfo`, as a `PrometheusRule`);
...
```
chambers/aws-eks/terraform/modules/platform/config/
...
[`shared/charts/podinfo`](../shared/charts/podinfo). The Prometheus Operator (kube-prometheus-stack)
