# Networking fundamentals, traced through Sanctum

TCP/IP, DNS, HTTP, and distributed networks underpin everything the platform does.
They matter because they separate people who click "create load balancer" from people who
can debug why a request hangs. This doc follows one request from a partner's client
to a Sanctum pod and names what happens at each layer, then covers the pieces that
...
- **Subnets** are AZ-scoped. Sanctum uses three tiers: public (ALB, NAT), private (EKS nodes), data (RDS, ElastiCache). Only the public tier has a route to the internet gateway.
...
## Distributed-network concepts that matter
