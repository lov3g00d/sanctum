# AWS Well-Architected, mapped to Sanctum

The Well-Architected Framework is six pillars plus a review process. Reciting the
pillar names is the easy part; the value is in making explicit tradeoffs between
them, because they conflict: more reliability costs money, more
security adds latency, more performance can hurt cost. Below each pillar lists the
design questions it asks, what Sanctum does about it, and where in this repo.

## 1. Operational Excellence

"Can you run, observe, and improve the system safely?"

- Everything is code: infra in `chambers/aws-eks/terraform/`, app config in `shared/charts/podinfo/`, pipelines in `.github/workflows/`. No console clicks that drift from source of truth.
...
- Identity: no long-lived AWS keys. CI authenticates with GitHub OIDC (`chambers/aws-eks/terraform/modules/github-oidc/`), workloads use IRSA per service account. This is the single highest-leverage AWS security control.
- Network: default-deny NetworkPolicy in `chambers/aws-eks/terraform/modules/platform/policies/cluster/`, private subnets for compute and data, security groups referencing other SGs rather than CIDRs.
