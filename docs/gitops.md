...
to deliver the app, notice a regression, and undo it without a human. In Sanctum
that something is ArgoCD, and the delivery custom resources it reconciles live in
[`shared/gitops/`](../shared/gitops/).
...
- **The GitOps layer tells ArgoCD what to deliver.** An `AppProject` scopes what
  ...
  environment into its own Application, syncing the `shared/charts/podinfo` Helm chart
...
are in [`shared/gitops/README.md`](../shared/gitops/README.md).
