# Cilium: the eBPF dataplane for Sanctum

Sanctum runs Cilium as the primary CNI on EKS. This replaces the packet path that a
...
- **L3-L7 policy.** CiliumNetworkPolicy filters on HTTP method and path, DNS names,
  ...
  `shared/charts/podinfo/templates/ciliumnetworkpolicy.yaml` allows `GET /metrics` from monitoring while
...
**replace** mode Cilium owns the dataplane and Service load-balancing outright. Sanctum
...
The Helm values that encode this (`chambers/aws-eks/terraform/modules/platform/cilium.tf`):
...
1. **Cluster comes up kube-proxy-free.** The EKS module (`chambers/aws-eks/terraform/modules/eks/main.tf`)
...
alerts, for example alerting on a rising `hubble_drop_total` for the sanctum namespace.

## How CiliumNetworkPolicy relates to the existing NetworkPolicies

The repo already ships Kubernetes NetworkPolicies in
`chambers/aws-eks/terraform/modules/platform/policies/cluster/networkpolicy.yaml`: default-deny, allow DNS egress, allow ingress to
podinfo, allow egress to the data stores. Cilium enforces those natively; adopting it does
not orphan them. `shared/charts/podinfo/templates/ciliumnetworkpolicy.yaml` layers on top rather than replacing them.
