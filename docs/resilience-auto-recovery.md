...
The goal is simple to state: "fail safely, recover automatically, keep the business
...
Sanctum exposes `/healthz` (liveness, always 200 while the process is up) and `/readyz`
...
Sanctum targets (illustrative, the point is to have numbers and defend them):
...
1. **Backup and restore** - restore infra + data in the DR region on demand. Hours of RTO, lowest cost. Sanctum's day-one posture.
