...
Cluster-infra layer for Sanctum: the shared controllers every workload depends on, plus
...
platform/
  ...
  cluster-posture.tf        sanctum namespace, NetworkPolicies, RBAC, quota, LimitRange
...
core resources with no engine dependency. The `sanctum` app namespace is owned here (PSA
...
### sanctum namespace: posture, secrets, ingress
...
| External Secrets | custom policy: `secretsmanager:GetSecretValue` + `ssm:GetParameter*`, scoped to `sanctum/*` |
...
|------|------|---------|-------------|
| `cluster_name` | string | - | Target EKS cluster (`sanctum-dev`, `sanctum-prod`) |
...
| `enable_cluster_posture` | bool | `true` | Deliver the sanctum namespace posture (NetworkPolicy, RBAC, quota, LimitRange) |
