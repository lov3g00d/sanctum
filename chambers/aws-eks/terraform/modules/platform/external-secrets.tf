...
# built-in policy, this grants only the reads ESO performs, and only under the sanctum/
...
data "aws_iam_policy_document" "external_secrets" {
  ...
  statement {
    ...
    resources = ["arn:aws:secretsmanager:${var.region}:${data.aws_caller_identity.current.account_id}:secret:sanctum/*"]
  }
  ...
  statement {
    ...
    resources = ["arn:aws:ssm:${var.region}:${data.aws_caller_identity.current.account_id}:parameter/sanctum/*"]
  }
}
...
resource "aws_iam_policy" "external_secrets" {
  ...
  description = "Scoped SecretsManager/SSM read access for External Secrets under sanctum/*"
