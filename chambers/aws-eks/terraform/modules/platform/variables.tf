variable "cluster_name" {
  description = "Name of the target EKS cluster (e.g. sanctum-dev, sanctum-prod)."
  type        = string
}
...
variable "region" {
  description = "AWS region the cluster runs in (eu-central-1 for Sanctum)."
  type        = string
}
...
variable "enable_cluster_posture" {
  description = "Deliver the sanctum app-namespace posture (namespace, NetworkPolicies, RBAC, quota, LimitRange)."
  type        = bool
  default     = true
}
