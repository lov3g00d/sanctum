...
Infrastructure for Sanctum on AWS and Cloudflare: reusable modules composed by a
...
terraform/
  modules/            reusable modules (raw AWS resources or pinned community modules)
    ...
    serverless-api/   Lambda + SQS trigger, IAM, log group (event-driven, standalone)
    platform/         Helm addon layer via helm_release + per-chart IRSA
  live/               Terragrunt env layer
    root.hcl          remote state (S3 with native locking) + generated provider, reads env.hcl
...
remote_state {
  backend = "s3"
  config = {
    bucket       = "sanctum-tfstate-<account-id>"
