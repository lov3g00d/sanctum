...
| [`kind-k8sgpt`](chambers/kind-k8sgpt/) | k8sgpt triaging a deliberately broken workload on kind: deterministic analyzers find the faults, `--explain` routes them through a local Ollama model, and the same engine is reachable via an MCP server (for Claude Code) and the operator (continuous scanning into Result CRDs) |
