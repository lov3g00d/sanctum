  ...
  values = [
    yamlencode({
      tetragon = {
        ...
        # relies on it to stay confined to the nimbus namespace.
...
# Enforcement example, scoped to the nimbus app namespace. TracingPolicyNamespaced
...
# MAY_READ=4.
resource "kubectl_manifest" "tetragon_enforce_nimbus" {
  # Also gated on cluster_posture: it owns the nimbus namespace this policy lands
  ...
  yaml_body = yamlencode({
    ...
    metadata = {
      name      = "nimbus-block-credential-access"
      namespace = "nimbus"
    }
    spec = {
      kprobes = [
  ...
  # The helm_release registers the CRDs; cluster_posture creates the nimbus namespace
