...
ArgoCD reconciles the podinfo Helm chart under `shared/charts/podinfo` into the cluster.
...
Bootstrapping is a single apply of `shared/gitops/` (project + ApplicationSet); from
...
an Application named `podinfo-<env>` that syncs `shared/charts/podinfo` with Helm value
...
`https://kubernetes.default.svc` and namespace `sanctum`, because the template
sets that destination and the `sanctum` AppProject exposes a single in-cluster
...
recording rules, scoped to `{namespace="sanctum"}` so each returns a single
series:
...
| File | What it is |
|------|------------|
| `appproject.yaml` | `AppProject sanctum`: source-repo allowlist, `sanctum` + `monitoring` destinations, least-privilege resource whitelists |
| `applicationset.yaml` | List generator over `[dev, prod]`, one Application per env syncing `shared/charts/podinfo` with its value files |
