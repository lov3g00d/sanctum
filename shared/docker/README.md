...
Container images for Sanctum, built for a small attack surface and non-root, read-only-friendly runtimes.
...
```sh
docker build -f docker/podinfo.Dockerfile -t sanctum/podinfo .
docker build -f docker/nginx/Dockerfile    -t sanctum/nginx .
```

## Scan

```sh
trivy image --severity HIGH,CRITICAL --ignore-unfixed sanctum/podinfo
