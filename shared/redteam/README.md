...
Purple-team validation for the Sanctum platform. Every artifact here is an attack
**attempt** paired with the Sanctum control that is supposed to stop it, wired so
...
built in `chambers/aws-eks/terraform/modules/platform/` works.
...
`chambers/aws-eks/terraform/modules/platform/`, `shared/charts/`, and `shared/gitops/`. It is run by hand when
...
| 05 | unsigned sanctum-ECR image | admission-deny | Kyverno verify-image-signatures (Enforce) |
...
- **05** verifies image signatures, and Kyverno `verify-image-signatures` is
  scoped to `*.dkr.ecr.eu-central-1.amazonaws.com/sanctum/*`. A generic public
  ...
  references the sanctum ECR repo with an unsigned tag to isolate the signature
  check.
...
```
./run-validation.sh --context my-sanctum-cluster
```

It applies each attempt into the `sanctum` namespace, checks the matching control
