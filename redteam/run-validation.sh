#!/usr/bin/env bash
#
# run-validation.sh - purple-team control validation for the Nimbus platform.
#
# AUTHORIZATION AND SCOPE
#   This is authorized self-validation of the operator's OWN reference
#   platform. Every manifest under attacks/ is an attack ATTEMPT that a Nimbus
...
#   nimbus namespace, checks that the matching control stopped each one, prints
...
eval_runtime_kill() {
  ...
  ns="$(field "$file" '.metadata.namespace')"; ns="${ns:-nimbus}"
  if ! out="$("${KUBECTL[@]}" apply -f "$file" 2>&1)"; then
...
echo "============================================================"
echo " Nimbus purple-team validation"
...
for f in "${ATTACK_FILES[@]}"; do
  base="$(basename "$f")"
  class="$(field "$f" '.metadata.labels."redteam.nimbus/control-class"')"
  technique="$(field "$f" '.metadata.annotations."redteam.nimbus/mitre"')"
  expected="$(field "$f" '.metadata.annotations."redteam.nimbus/expected"')"
