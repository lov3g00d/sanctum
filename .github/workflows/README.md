...
GitHub Actions workflows for Sanctum, built around one idea: **shift-left**. Every
...
| `terraform.yml` | PR / push touching `chambers/aws-eks/terraform/**` | Plan on PR (commented), apply on merge (gated). |
...
```
kubectl -n sanctum rollout undo deployment/podinfo
```
...
Path-filtered to `chambers/aws-eks/terraform/**` so it only runs when infra changes. Matrix over the
...
`chambers/aws-eks/terraform/`). That overlap is intentional. `ci.yml` gates every PR; this
...
| `AWS_DEPLOY_ROLE_ARN` | `arn:aws:iam::123456789012:role/sanctum-gha-deploy` |
| `AWS_TF_PLAN_ROLE_ARN` | `arn:aws:iam::123456789012:role/sanctum-gha-tf-plan` |
| `AWS_TF_APPLY_ROLE_ARN` | `arn:aws:iam::123456789012:role/sanctum-gha-tf-apply` |
| `DEV_BASE_URL` | `https://dev-api.sanctum.example.com` |
